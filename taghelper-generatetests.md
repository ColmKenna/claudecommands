---
description: TagHelper Test Generation Instructions
---
...
**REQUIRED FORMAT:** `Should_ExpectedBehavior_When_Scenario`

**Examples:**
- `Should_SetNameAttribute_When_WithValidName`
- `Should_ThrowArgumentException_When_ForIsEmpty`
- `Should_GenerateHiddenInput_When_WithCheckboxType`
- `Should_AddToFormContext_When_InEndOfFormMode`

**NEVER USE:**
- Generic names like `Test1`, `BasicTest`, `ValidInput`
- Underscores without the three-part structure
- CamelCase without underscores
- Old format like `MethodName_Condition_ExpectedOutcome`
...
```csharp
[Fact]
public async Task Should_ExpectedBehavior_When_Scenario()
{
...
```csharp
[Fact]
public async Task Should_GenerateExpectedOutput_When_WithValidInput()
{
...
```csharp
[Fact]
public void Should_GiveNamePrecedence_When_WithNameAndFor()
{
...
```csharp
[Fact]
public void Should_UseForName_When_NameIsNull()
{
    // Fallback behavior
}
...
```csharp
[Fact]
public async Task Should_AddHiddenInputToPostElement_When_CheckboxInlineMode()
{
    // ViewContext.CheckBoxHiddenInputRenderMode = CheckBoxHiddenInputRenderMode.Inline
}

[Fact]
public async Task Should_AddHiddenInputToFormContext_When_CheckboxEndOfFormMode()
{
...
```csharp
[Fact]
public void Should_ThrowInvalidOperationException_When_WithoutFormContext()
{
    // Missing required parent state
}

[Fact]
public void Should_AddStateToFormContext_When_WithFormContext()
{
...
```csharp
[Fact]
public async Task Should_CallGenerateCheckBox_When_WithExpectedParameters()
{
...
```csharp
[Fact]
public void Should_ThrowInvalidOperationException_When_WithInvalidConfiguration()
{
...
```csharp
[Fact]
public async Task Should_WriteToPostElement_When_WithCheckbox()
{
    await tagHelper.ProcessAsync(context, output);
...
public void Should_SetTypeAttribute_When_WithInputType(string inputType, string expectedType)
{
...
```csharp
[Fact]
public void Should_GenerateFullOutput_When_WithCompleteInput()
{
    tagHelper.Process(context, output);
...
- [ ] All test names follow `Should_ExpectedBehavior_When_Scenario` format
...
```csharp
[Fact]
public async Task Should_AddHiddenToPostElement_When_WithCheckboxInline()
{
    var metadataProvider = new EmptyModelMetadataProvider();
    var htmlGenerator = new TestableHtmlGenerator(metadataProvider);
...
```csharp
[Fact]
public async Task Should_GenerateAntiforgeryToken_When_WithPost()
{
...
```csharp
[Fact]
public void Should_UseModelPropertyName_When_WithFor()
{
    var metadataProvider = new EmptyModelMetadataProvider();
    var htmlGenerator = new TestableHtmlGenerator(metadataProvider);
    var modelExplorer = metadataProvider.GetModelExplorerForType(typeof(string), "value");
...
```csharp
[Fact]
public void Should_UseFormData_When_WithParentFormContext()
{
